...
public class ClasssTest {
    ...
    public static void main(String[] args) {
        ClasssTest garen =  new ClasssTest();
        garen.name = "盖伦";
        //必须有一个对象才能调用
        garen.die();

        ClasssTest teemo =  new ClasssTest();
        teemo.name = "提莫";

        //无需对象，直接通过类调用
        ClasssTest.battleWin();
    ...
    //能否直接调用？ 为什么？
    //--------------------------------
    //不能调用。因为battleWin是静态方法，而die是非静态方法，静态方法不能调用非静态方法。
    //非静态方法，要在实例化后，采用对象名.方法名的方式进行调用。
